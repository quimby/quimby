...
public:
	...
	unsigned int loadParticles(const Vector3f &lower, const Vector3f &upper,
	...
	static void create(std::vector<SmoothParticle> &particles,
		...
		for (size_t iX = 0; iX < blocks_per_axis; iX++) {
			...
			for (size_t iY = 0; iY < blocks_per_axis; iY++) {
				...
				std::cout << iX << " " << iY << " " << indices.size() << " / "
						<< (first_out - first_in) << std::endl;
				...
				for (size_t iZ = 0; iZ < blocks_per_axis; iZ++) {
					...
					for (size_t i = 0; i < indices.size(); i++) {
						...
						if (!block_box.contains(p.position))
							continue;
						block.count++;
						block.margin = std::max(block.margin,
								p.smoothingLength);
		...
		std::cout << "Count: " << count << " / " << total_count << std::endl;
	}
};
