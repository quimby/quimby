...
public:
	...
	SPHVisitor(const SmoothParticle &particle, size_t reslution_kpc) :
			lastX(-1), lastY(-1), lastZ(-1), particle(particle), reslution_kpc(
					reslution_kpc) {
	}
...
int sph_dump(Arguments &arguments) {
	...
	for (int iP = 0; iP < s; iP++) {
		...
		Vector3f b, p;
		size_t x_min = clamp(
				(int) ::floor((sp.position.x - r) / resolution_kpc), 0,
				(int) samples - 1);
		size_t x_max = clamp((int) ::ceil((sp.position.x + r) / resolution_kpc),
				0, (int) samples - 1);
		for (size_t x = x_min; x <= x_max; x++) {
			...
			b.x = SmoothParticle::kernel(sp.bfield.x, p.x, sp.position.x,
					sp.smoothingLength);
			float r_yz = sqrt(r_yz2);

			size_t y_min = clamp(
					(int) ::floor((sp.position.y - r_yz) / resolution_kpc), 0,
					(int) samples - 1);
			size_t y_max = clamp(
					(int) ::ceil((sp.position.y + r_yz) / resolution_kpc), 0,
					(int) samples - 1);
			for (size_t y = y_min; y <= y_max; y++) {
				...
				size_t z_min = clamp(
						(int) ::floor((sp.position.z - r_z) / resolution_kpc),
						0, (int) samples - 1);
				size_t z_max = clamp(
						(int) ::ceil((sp.position.z + r_z) / resolution_kpc), 0,
						(int) samples - 1);
				for (size_t z = z_min; z <= z_max; z++) {
					p.z = z * resolution_kpc;
					b.z = SmoothParticle::kernel(sp.bfield.z, p.z,
							sp.position.z, sp.smoothingLength);
					grid.get(x, y, z) += b;
				}
			}
		}
